...
import {
    PostComponent
} from '../posts/post/post.component';
import {
    environment
...
@Component({
  selector: 'app-posts-unfv2',
  templateUrl: './posts-unfv2.component.html',
  styleUrls: ['./posts-unfv2.component.css']
})
export class PostsUnfv2Component implements OnInit {
    ...
    subscription2:any;
    id_noOptions:string;
    id_filterChange:string;
    val_noOptions:string;
    val_filterChange:string;
    ...
    constructor(private postsService: PostsService, private authenticationService: AuthenticationService, private globalService: GlobalService, private flashMessagesService: FlashMessagesService, private stompService: StompService, private activatedRoute: ActivatedRoute, private router: Router) {
        ...
        //const noOptionsFound$ = Observable.fromEvent(elem, 'click');
        //const filterInputChanged$ = Observable.fromEvent($('#59669ee7ea025c61ad3724ee'), 'filterInputChanged');
    ...
    cargaFormulario() {
        if (!this.isFormLoaded) {
            this.isFormLoading = true;
            this.postsService.getVariablesFormulario()
            .finally(() => console.log(document.getElementById('59669ee7ea025c61ad3724ee')) )
            .subscribe(
                res1 => {
    ...
    public onNoOptionsFound(value,id){
        this.id_noOptions=id;
        this.val_noOptions=value;
    }

    public onFilterInputChanged(value,id){
        this.id_filterChange=id;
        this.val_filterChange=value;
    }

    public onClose(){
        this.id_filterChange="";
        this.val_filterChange="";
    }
    ...
    addOptionEnabler(id){
        if(this.id_filterChange == this.id_noOptions && this.val_filterChange == this.val_noOptions && this.id_noOptions+'button' == id &&
            this.id_filterChange!="" && this.val_filterChange!=""){
            return false;
        }else{
            return true;
        }
